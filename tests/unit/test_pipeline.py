...
def _invoke(user_message: str, **extra) -> dict:
...
class TestSingleTickerRoute:
    def test_single_route_set(self):
        result = _invoke("AAPL")
        assert result["route"] == "single"

    def test_single_final_message_set(self):
        result = _invoke("AAPL")
        assert result.get("final_message") is not None
        assert len(result["final_message"]) > 0

    def test_single_tickers_contains_aapl(self):
        result = _invoke("AAPL")
        assert "AAPL" in result["tickers"]

    def test_single_data_by_ticker_populated(self):
        result = _invoke("AAPL")
        assert "AAPL" in result.get("data_by_ticker", {})

    def test_single_analyst_outputs_populated(self):
        result = _invoke("AAPL")
...
class TestComparisonRoute:
    def test_comparison_route_with_vs(self):
        result = _invoke("AAPL vs MSFT")
        assert result["route"] == "comparison"

    def test_comparison_route_with_compare(self):
        result = _invoke("compare AAPL MSFT")
        assert result["route"] == "comparison"

    def test_comparison_both_tickers_present(self):
    ...
    def test_comparison_final_message_set(self):
        result = _invoke("AAPL vs MSFT")
        assert result.get("final_message") is not None

    def test_comparison_data_by_ticker_has_both(self):
        result = _invoke("compare AAPL MSFT")
...
class TestNewsOnlyRoute:
    def test_news_only_route_set(self):
        result = _invoke("news TSLA")
        assert result["route"] == "news_only"

    def test_news_only_final_message_set(self):
        result = _invoke("news TSLA")
        assert result.get("final_message") is not None

    def test_news_only_tickers_set(self):
        result = _invoke("news TSLA")
...
class TestInvalidRoute:
    def test_invalid_route_set(self):
        result = _invoke("!!!")
        assert result["route"] == "invalid"

    def test_invalid_final_message_contains_couldnt(self):
        result = _invoke("!!!")
        assert "couldn't identify" in result["final_message"].lower()

    def test_invalid_errors_populated(self):
        result = _invoke("!!!")
        errors = result.get("errors", [])
        assert any("route:invalid" in e for e in errors)

    def test_invalid_no_tickers(self):
        result = _invoke("!!!")
        assert result["tickers"] == []

    def test_numeric_only_is_invalid(self):
...
class TestReflectionLoop:
    ...
    def test_reflection_loop_limited_by_max_iterations(self):
    ...
    def test_reflection_loop_analyst_output_reflects_iterations(self):
    ...
    def test_normal_critic_passes_first_time(self):
